...
export function createSession(folder, tool, name, extra = {}) {
  ...
  const codexAutomationMode = tool === 'codex'
    ? normalizeCodexAutomationMode(extra.codexAutomationMode)
    : null;
  const initialNaming = resolveInitialSessionName(name);
  const now = new Date().toISOString();
  const metas = loadSessionsMeta();
  const baseName = initialNaming.autoRenamePending && tool === 'codex'
...
export function sendMessage(sessionId, text, images, options = {}) {
  ...
  console.log(
    `[session-mgr] Spawning tool=${effectiveTool} model=${options.model || 'default'} ` +
    `effort=${options.effort || 'default'} thinking=${!!options.thinking} ` +
    `codexMode=${spawnOptions.codexAutomationMode || 'n/a'}`
  );
